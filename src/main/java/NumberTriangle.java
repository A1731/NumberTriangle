...
public class NumberTriangle {
    ...
    for (int i = 0; i < path.length(); i++) {
        ...
        if (ch == 'l') {
            if (cur.left == null) {
                throw new IllegalArgumentException("Path goes left out of bounds at index " + i);
            }
            cur = cur.left;
        } else if (ch == 'r') {
            if (cur.right == null) {
                throw new IllegalArgumentException("Path goes right out of bounds at index " + i);
            }
            cur = cur.right;
        } else {
            throw new IllegalArgumentException("Invalid path char: " + ch + " (only 'l' or 'r')");
        }
    }
    return cur.getRoot();
}
